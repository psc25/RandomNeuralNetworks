...
for mi in range(len(mm)):
    ...
    for Ji in range(len(JJ)):
        ...
        plt.legend()
        plt.ylim([0.0, 0.04])
        plt.savefig("fokker_data/fokker_sv_DT_learn_" + str(m) + "_" + str(J) + ".png", bbox_inches = 'tight', dpi = 500)
        plt.show()
        plt.close(fig)
        
        print("DT learned for m = " + str(m) + ", J = " + str(J) + ", N = " + str(N) + ", in " + '{:.4f}'.format(tms[mi, Ji]) + "s: in-sample " + '{:.4f}'.format(err[mi, Ji, 0]) + ", out-of-sample " + '{:.4f}'.format(err[mi, Ji, 1]))
        
        np.savetxt("fokker_data/fokker_sv_DT_err_" + str(mm[mi]) + ".csv", err[mi])
        np.savetxt("fokker_data/fokker_sv_DT_tms_" + str(mm[mi]) + ".csv", tms[mi])
        np.savetxt("fokker_data/fokker_sv_DT_u1p_" + str(mm[mi]) + ".csv", u1p[mi])