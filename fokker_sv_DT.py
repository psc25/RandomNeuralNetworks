...
for mi in range(len(mm)):
    ...
    for Ni in range(len(NN)):
        ...
        plt.legend()
        plt.ylim([0.0, 0.04])
        plt.savefig("fokker_data/fokker_sv_DT_learn_" + str(m) + "_" + str(N) + ".png", bbox_inches = 'tight', dpi = 500)
        plt.show()
        plt.close(fig)
        
        print("DT learned for m = " + str(mm[mi]) + ", N = " + str(NN[Ni]) + ", in " + '{:.4f}'.format(tms[mi, Ni]) + "s: in-sample " + '{:.4f}'.format(err[mi, Ni, 0]) + ", out-of-sample " + '{:.4f}'.format(err[mi, Ni, 1]))
